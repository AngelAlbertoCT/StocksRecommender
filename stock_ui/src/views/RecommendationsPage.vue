  ...
  <section class="bg-gray-50 min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-8 text-center">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Recomendaciones por perfil</h1>
        <p class="text-gray-600">Acciones seleccionadas según tu perfil de inversión</p>
      </div>
      ...
      <div v-else-if="store.stocks.length === 0" class="text-center py-12">
        <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-gray-100 mb-4">
          <svg class="h-6 w-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h3 class="text-lg font-medium text-gray-900 mb-1">No hay recomendaciones</h3>
        <p class="text-gray-500">Selecciona un perfil para ver recomendaciones personalizadas</p>
      </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">% Ganancia</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span 
                    :class="{
                      'text-green-600': stock.gain_percent > 0,
                      'text-red-600': stock.gain_percent < 0,
                      'text-gray-600': stock.gain_percent === 0
                    }"
                    class="font-medium"
                  >
                    {{ stock.gain_percent !== undefined ? `${stock.gain_percent > 0 ? '+' : ''}${stock.gain_percent.toFixed(2)}%` : 'N/A' }}
                  </span>
                </td>
              </tr>
            </tbody>
